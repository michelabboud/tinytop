...
The Settings dialog opens from the left rail and uses five category tabs so adding a feature does not turn configuration into one long scrolling page:

- **General** contains `This Browser` and `This Daemon`. Browser controls set the active theme, graph mode, and history window for the current profile. Daemon controls hold defaults stored in SQLite, including refresh, database budget, process count, redaction, thresholds, and visible sections.
- **History** contains the Rust retention ladder: L1 raw and L2 one-minute retention, optional L3 five-minute and L4 hourly tiers, filesystem detail cadence, process fast-history horizon, archive options, and disk thresholds. `History hours` / `Rollup days` remain derived compatibility mirrors. Bun shows the runtime-only explanation and saves without `retentionLadder`.
- **Metrics** is present only when the daemon advertises `GET /api/otel/metrics`. It groups the daemon's registry in registry order and offers per-family select-all/none without storing a wildcard.
- **Thermals** is present only when the daemon advertises thermal capability. It contains the opt-in CPU thermal collector and additional CPU hwmon chip allow-list.
- **Advanced** contains the expert OpenTelemetry endpoint/protocol/header-variable/resource controls and the versioned raw settings document.

Every registered metric exports by default. A checked Metrics box therefore means enabled; unchecking it adds the explicit name to `otel.disabledMetrics`. Storing the disabled set makes a metric introduced by a later TinyTop release enabled automatically instead of leaving upgraded hosts in silent shadow state. A disabled name unknown to this build is preserved during save and displayed as **unknown on this version (inert)**, so one fleet document can round-trip through machines running different releases without erasing newer choices.

The Advanced document editor is seeded from `GET /api/settings/export`. **Validate** sends the document to the daemon's existing import dry-run and displays the daemon's authoritative errors or changed keys; the browser deliberately does not duplicate server validation. **Apply** remains disabled until that exact text passes, and editing even one character disables it again, preventing an old verdict from authorizing a document the daemon never checked.
