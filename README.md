...
| `retentionLadder.detailIntervalSec` | `60` | Filesystem check interval; schema v4 stores filesystem detail only when it changes; 15–3,600 seconds |
...
The current Rust SQLite implementation stores typed metric rows plus interned host identity and GPU adapters, maintains the four-tier ladder, records filesystem changes/presence events, GPU samples, and typed process detail, and exposes the additive history endpoints above. A migrated v1 file retains its old SQLite page slack until the operator runs `tinytop-agent db vacuum`; schema migration does not reclaim that slack automatically.

## GPU

TinyTop's Linux GPU backend reads only kernel interfaces: DRM sysfs for adapter identity, supported busy and VRAM counters; `/proc/<pid>/fdinfo` for DRM engine deltas and per-process attribution; and the GPU node's own hwmon `temp1_input` for temperature. It never starts a subprocess and never links a vendor library (ADRs 0022 and 0025). Native Windows and macOS GPU backends are Task 16 and are not implemented yet.

Detection is the gate. DRM entries must be real `card` plus digits devices with a readable PCI vendor; connector and render nodes are ignored. Adapters are re-detected on the slow tick for hotplug. When no adapter is detected, TinyTop performs no GPU sampling, writes no GPU rows, and omits `gpus` from snapshots. WSL2 therefore reports no guest GPU because its guest kernel exposes no DRM adapter.

Where `gpu_busy_percent` works, TinyTop prefers it. Otherwise it derives utilization from cumulative fdinfo engine counters: it sums client deltas for each engine, divides by engine capacity and the monotonic scan interval, and reports the busiest engine capped at 100 percent—the same definition used by Task Manager. The first fdinfo tick has no interval and therefore no busy value. Proprietary NVIDIA adapters are identity-only in this phase, so their metric fields remain absent.

Linux fdinfo access follows the daemon user's `/proc` permissions. The daemon normally sees its own processes; on a multi-user host, fdinfo-derived adapter busy is a lower bound and per-process GPU percentages cover only readable processes. Running the daemon as root or granting `CAP_SYS_PTRACE` broadens visibility, but TinyTop never asks for either privilege.
