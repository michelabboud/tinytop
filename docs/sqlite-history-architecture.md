...
Each insert rebuilds only its affected L2 minute, including all v1 minimum and maximum columns. A completed bucket is otherwise frozen: pruning never rebuilds it. If a late or clock-skewed insert lands behind an L3 or L4 fold watermark, the insert recomputes that minute and then re-folds the already-promoted five-minute and hourly ancestors. Persisted `l3Enabled`/`l4Enabled` state prevents this repair path from writing a disabled tier after it has retained an old watermark.
