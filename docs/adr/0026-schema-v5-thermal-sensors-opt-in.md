...
## Known limitation 2026-08-31 — `stable_id` still forks when two SAME-NAME chips swap sysfs order

Raised as a MED finding by the blind review of T17 (`docs/fleet/tinytop/2026-08-31-t17-luna-review.md` in the Fabulous repository, finding 4) and **validated at source by the orchestrator**. It is recorded here rather than fixed because the fix is a schema-visible identity change, not a bug patch.

**The residual hole.** Decision 1 deliberately keeps the `hwmonN` directory index out of `stable_id`, because that index is not stable across hosts or boots (Context) — that part works. But the disambiguator it uses instead, `<k>` = "the 0-based count of earlier chips with the same `name` in sorted-path order", is itself derived from **ordering**, not from identity. On a host with two chips sharing one `name` — the dual-socket `coretemp` case decision 1 explicitly set out to serve — a reboot that reorders those two sysfs paths swaps their `<k>`, so each socket's history silently continues under the other's `stable_id`. The series do not break; they **cross**, which is the more expensive failure because nothing looks wrong.

**Why it is not blocking, stated as a measurement and not a hope.** `<k>` is only load-bearing when a `name` occurs more than once. With exactly one chip of a given name, `<k>` is 0 in every possible ordering, so the id is order-independent by construction. The fleet's measured fixtures (§A of the T17 fact sheet) have exactly one `coretemp` each — sheep `hwmon1`, trashcan `hwmon0` — and no fleet host is dual-socket. The defect is therefore unreachable on every machine TinyTop currently runs on, and reachable the day one dual-socket host appears.

**The proper fix, deferred to backlog.** Derive the disambiguator from the chip's **stable device path** rather than from scan order — the same trick ADR 0025 decision 2 already uses for GPUs, where `stable_id` is `pci-<PCI_SLOT_NAME>` (`pci-0000:02:00.0`) and falls back to `card<N>` only when no slot name exists. A `coretemp` chip's `hwmonN/device` symlink resolves to exactly such a stable path, so the same idiom applies with no new concept. This is deliberately **not** done in T17: changing `stable_id`'s grammar rewrites the identity of every already-stored sensor, which needs a migration that maps old ids to new ones (or an accepted history fork at upgrade) — a decision that deserves its own ADR, taken when a host actually needs it.

**Until then:** the grammar in decision 1 stands as shipped, and this limitation is the honest reading of it.
