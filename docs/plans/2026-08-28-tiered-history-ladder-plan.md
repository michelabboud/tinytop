...
- [x] **Landed** 2026-08-28 as run 547 (`ari-sol-deep`) `b38e52e` + fix1 run 551 `01fbaa2` + fix2 run 553 (`ari-spark`, 73 s) `2b0e939` after luna run 548; merged `0a46887` → **v0.2.9** `35165be`. Gates green outside the sandbox (Bun 102/102, Rust 16 suites).
...
- [x] **Landed** 2026-08-28 as run 557 (`ari-sol-deep`, after run 554 escalated correctly on the impossible `auto` row → ruling `287fc25`) `80c1c33` + fix1 run 563 `c04a62d` after luna run 561 (P1 `limit: None` 120 vs 10,000 → `effective_points_limit`); merged `ede6851` → **v0.2.11** `b4606a0`. Fable's curl transcript in the `80c1c33` message; live `auto` proof on main: 30 d @ 10,000 → `5m`, @ 100 → `1h`, 800 d → `1h`. Gate on main green (Bun 145/145, Rust 16 suites).
...
- [ ] **Step 1: failing Bun tests:** `HISTORY_WINDOWS` has keys `live,15m,1h,6h,24h,7d,30d,90d,1y,all` with sources `raw,raw,raw,auto,auto,auto,auto,auto,auto,auto` (**amended 2026-08-28 → T5-fix1 F5**; T5 built the original `…,rollup,rollup,rollup,rollup,5m,1h,auto`, which truncates `30d` to 6.9 d — spec §11); `validateRetentionLadder` mirrors each server message (copy the exact strings from `retention_ladder.rs`); `historyWindowFor("1y", coverageWithL4Disabled)` returns `{disabled:true, reason:"retentionLadder.l4.enabled"}` and `historyWindowFor("90d", coverageWithL3DisabledL4Enabled)` is NOT disabled (auto degrades to L4) — amended 2026-08-28 with F5.
...
- [x] **Landed** 2026-08-28 as run 555 (`ari-sol`, escalated AFTER finishing on `static_relative_path` — the brief's "two lines" claim) `ea8c6f9` + fix1 run 562 `9770dbf` (F1–F7 incl. presets ≥6h via `auto`, Bun parity + honesty) after luna run 558; merged `75b1541` → **v0.2.10** `4237bac`. Fable's Playwright checklist on main v0.2.11: ladder group renders · L2=6 → `retentionLadder.l2.keepDays must be between 7 and 3650; observed 6` · L4 forever saves (`keepDays 0`) · coverage card shows four tiers · console clean (Fabulous `docs/fleet/tinytop/2026-08-28-t5-acceptance-checklist.md`).
...
**Files:** Modify `writer.rs` (routes `GET /api/settings/export`, `POST /api/settings/import` with `dryRun`; `wouldDelete` computed from tier counts older than the candidate horizons via one `SELECT COUNT(*)` per tier), `lib.rs` (`count_rows_older_than(tier, cutoff_ms)`), `main.rs` (`config export [--out FILE]`, `config import FILE [--dry-run]`), `app.js`/`index.html` (Export/Import buttons; **replace T5's approximate `wouldDelete` with the dry-run response and delete the "approx." label**); Tests **(amended 2026-08-29 at dispatch — the logic lives in a new store module `tinytop-store/src/settings_transfer.rs` so the HTTP handler and the CLI share it; the agent crate's `AppState` is private and the sandbox cannot bind ports, so the route tests use the existing port-free `oneshot` harness):** `agent/crates/tinytop-store/tests/settings_transfer.rs` (export shape + no secret-shaped key; dry-run diff lists `changedKeys` and `wouldDelete`; import applies; unknown top-level key refused; `tinytopConfigVersion: 2` refused naming max 1; unknown keys inside `settings` → `warnings[]`; import under disk pressure refuses growth, also when the pressure arrives between plan and apply), `writer.rs` `#[cfg(test)]` (export headers; dry-run does not apply; import applies + records `settingsChange{source:"import"}`; 400 on a newer document), `agent/crates/tinytop-agent/tests/cli_config.rs` (round trip; `--out` never overwrites; exit 1 on errors; missing/v0 database refused); Docs `README.md`, `GUIDE.md`, `docs/sqlite-history-architecture.md`, `CHANGELOG.md`. Brief: `briefs/T10.md`.
...
**Amended 2026-08-29 (Fable, at T11 dispatch from `v0.4.1`; the brief `briefs/T11.md` is authoritative):** (a) `tinytop-agent` has no `[lib]` target, so `tests/otel_export.rs` is not created — the exporter's tests live in `otel.rs`/`writer.rs` `#[cfg(test)]`; they are port-free except one `serve_`-prefixed socket test (the lane sandbox cannot bind; the outside gate proves it), the failure path targets `127.0.0.1:1`. (b) The exporter is driven by the daemon's own loop (`ManualReader` + `PushMetricExporter`, synchronous gauges recorded at export time, 5 s settings tick, 10 s export timeout) — never the SDK's `PeriodicReader`; the latest snapshot reaches it through a `tokio::sync::watch` channel published by `collect_and_store`. (c) `DashboardSettings::from_document` takes `persisted: Option<&DashboardSettings>` so an absent `otel` block keeps the persisted value exactly as an absent `retentionLadder` does; `tinytopConfigVersion` stays 1. (d) `HistoryCoverage.otel` is an `Option` the agent fills from its status; `db stats` reports the env-var NAME and its presence only. (e) `Cargo.lock` changes are expected; `opentelemetry-proto`/`prost` may be added as dev-dependencies at the tree's versions if the exporter crate does not re-export the proto types. (f) **11:40Z, after run 625 escalated correctly:** `opentelemetry_sdk` is pinned with `features = ["metrics", "experimental_metrics_custom_reader"]` — in 0.32 the caller-driven reader (`ManualReader`, `MetricReader`) exists only behind that experimental gate, while `PeriodicReader` is a `std::thread` + `futures_executor::block_on` design that cannot host the async reqwest client inside the tokio daemon and hides export failures; the gate is accepted as an upgrade-deliberately risk (exact pins) and stated in the vetting report. Re-dispatched as `tinytop-ladder-t11b` inside run 625's worktree (WIP commit of the partial work on the lane branch).
...
- ~~**T5's brief allowed exactly two `writer.rs` lines**~~ **RESOLVED 2026-08-28 — T5 escalated on this, correctly (run 555):** this plan's `ladder-rules.js` bullet below already named `writer.rs:static_relative_path`; the BRIEF narrowed it to the route + `include_bytes!` arm, so the module 404s under the Rust daemon. T5-fix1 F1 adds the arm + a `serve_contract` assertion. The lane's three plan-silent findings (pressure hedge `active ?? pressure` bypasses on `{active:false, pressure:true}` → F2; an unavailable default preset stays selected → F3; Bun's `settings.ts` stops at `24h` and drops `retentionLadder` → F4, spec §13 amended) are all real and in the same fix round.
- **Observed 2026-08-28 (a dashboard defect, pre-existing for `30d`; → T5-fix1 F5, decided 23:00Z; spec §11 amended):** `app.js` fetches rollup presets as ONE page of ≤ `MAX_HISTORY_PAGE_SIZE` (10,000) buckets — `30d` at 1 m holds 43,200 buckets, so the chart silently shows only the newest 6.9 d; `7d` loses its oldest 80 minutes; §11's `90d` at 5 m (25,920) would show 34.7 d. The fix is the ladder itself: every preset from `6h` up requests `source=auto&limit=10000` and renders by the returned `source`/`resolutionMs` (auto gives 6h → L2, 24h → L2, 7d → L3, 30d → L3, 90d → L4, 1y → L4 — every range complete in one page; a disabled finer tier degrades to the next instead of greying the button).
- **Noted 2026-08-28 for T6 (luna 561 P2, true and repo-wide):** axum's `Query` rejection names the offending parameter but not its observed value (`limit=abc` → "limit: invalid digit"), on every endpoint since before the ladder. T6 adds one `QueryRejection` mapper in `writer.rs` that echoes the parameter AND the value, with a test — not a T4 fix.
