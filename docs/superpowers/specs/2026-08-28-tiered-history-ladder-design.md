...
The check runs at daemon start and then every `intervalMinutes`; an undeterminable measurement keeps the last known state (ADR 0020).

**Cold export (hourly, when `archive.cold`):** for each month `M` in `archive.metric_rollups_1h` with `M ≤ now − coldAfterMonths` and `M > coldExportedUntilMonth`: write `<dir>/tinytop-1h-YYYY-MM.csv.gz` (header = the table's column names in DDL order; rows ordered by `bucket_start_ms`; RFC 4180; gzip level 6) to a `.tmp` name, fsync, compute sha256, **re-read the gzip and count rows + compare the first/last `bucket_start_ms`**, rename into place, write `.sha256` sidecar (`<hex>  <filename>` — `sha256sum -c` compatible), upsert `archive_manifest`, advance `coldExportedUntilMonth`. A month is eligible only after every hour has expired from main: `end_of_month_ms(M) + l4_keep_ms + 86_400_000 ≤ now_ms`, and (d) main holds no rows for `M`; if L4 is disabled or kept forever, no month is exportable. Cold export **never deletes** from the archive DB in this spec (a future `archive.coldPrunesQueryable` flag may; not now).
...
- Config: `GET /api/settings/export` → `{"tinytopConfigVersion":1,"exportedAtMs":…,"agentVersion":"…","settings":{…DashboardSettings…}}` with `Content-Disposition: attachment; filename="tinytop-settings-YYYYMMDD-HHMM.json"`. `POST /api/settings/import` body = that document; `?dryRun=true` returns `{valid, errors[], changedKeys[], wouldDelete:{l1Rows,l2Buckets,l3Buckets,l4Buckets}}` without applying; without `dryRun` it validates (all §5 rules), applies via `put_settings`, runs `maintain_history`, records marker `settingsChange` with `{"source":"import","changed":[…]}`. Unknown top-level keys are rejected (`tinytopConfigVersion` > 1 → error naming the max supported). **Amended 2026-08-29 (T10 dispatch, additive):** the dry-run response also carries `warnings[]` (unknown keys *inside* `settings` are ignored exactly as `PUT /api/settings` ignores them, but each is listed as `settings.<path>: unknown key ignored`) and `wouldDelete.snapshotJsonRows` (blobs the candidate `snapshotJsonKeepMinutes` would strip); a dry-run always answers `200` with `valid:false` + `errors[]` for a bad document, a real import answers `400 {"error"}` for one and `200 {applied:true, changedKeys, wouldDelete, settings}` on success; `wouldDelete` counts are taken with the prune predicates (`captured_at_ms < cutoff`; `bucket_start_ms + resolution ≤ cutoff`) under the candidate horizons, a disabled tier or a forever L4 counts 0, and with `archive.queryable` the L4 number is what moves out of `main`.
...
`DashboardSettings.otel` block: `{ "enabled": false, "endpoint": "http://127.0.0.1:4318/v1/metrics", "protocol": "http/protobuf", "intervalSec": 60, "headersEnvVar": "TINYTOP_OTEL_HEADERS", "serviceName": "tinytop", "resourceAttributes": {} }`. Secrets never live in settings: headers (e.g. auth) are read from the environment variable named by `headersEnvVar` (`k1=v1,k2=v2`, OTLP style) only when the exporter pipeline is (re)built — on enable and on a change of the `otel` block — never while the exporter is disabled; the settings export therefore never contains a secret.

Metrics (OTel semantic conventions where they exist; all gauges; unit in brackets): `system.cpu.utilization` [1] · `system.memory.utilization` [1] · `system.memory.usage` [By] (attr `state=used`) · `system.memory.limit` [By] · `system.paging.utilization` [1] (attr `state=used`; this is the semconv name for swap) · `system.cpu.load_average.1m` / `.5m` / `.15m` [{thread}] · `system.filesystem.utilization` [1] (attrs `mountpoint`, `type`) · `system.filesystem.usage` [By] (attrs `mountpoint`, `type`, `state=used|free`) · product-specific under the `tinytop.` prefix: `tinytop.load.percent` [%], `tinytop.pressure.some` / `.full` [%] (attr `resource=cpu|memory|io`, emitted only when the collector reports pressure). Resource: `service.name`, `service.version` = agent version, `host.name` = hostname, plus `resourceAttributes`. **Amended 2026-08-29 (T11-fix1):** `tinytop.load.percent` and `tinytop.pressure.some` / `.full` use `[%]` units, and `resourceAttributes` keys are ≤ 64 characters.

Exporter: `opentelemetry` 0.32.0 / `opentelemetry_sdk` 0.32.1 / `opentelemetry-otlp` 0.32.0, HTTP/protobuf, with the SDK's `ManualReader` behind `experimental_metrics_custom_reader`, driven by the daemon's own task (the SDK's `PeriodicReader` hosts a thread); observed values = the latest snapshot at export time (no re-collection). Runs in its own tokio task; export failure increments `otelExportFailures` (exposed in `/api/history/coverage` under `otel: {enabled, endpoint, lastSuccessMs, failures}`) and logs at `warn` with the endpoint and error, never more than once per minute; never blocks or delays collection. Bun runtime: no OTel (documented, like retention). **Amended 2026-08-29 (T11 dispatch):** absent from a settings document, the `otel` block keeps its persisted value (the same rule as `retentionLadder`) and `tinytopConfigVersion` stays 1 — the block is additive; the coverage block also carries `intervalSec`, `lastFailureMs` and `lastError` (an error string, never a header value); `db stats` reports `headersEnvVar` and whether it is set, never its value.

**Amended 2026-08-29 (Phase 4 close):** (a) headers are read from the environment only when the exporter pipeline is (re)built — on enable or an `otel`-block change — and never while disabled; (b) the exact pins are `opentelemetry` 0.32.0, `opentelemetry_sdk` 0.32.1, and `opentelemetry-otlp` 0.32.0, using the SDK `ManualReader` behind `experimental_metrics_custom_reader` and the daemon task because `PeriodicReader` hosts a thread; (c) `system.filesystem.usage` [By] has attributes `mountpoint`, `type`, and `state=used|free`.
...
`db stats` prints the ladder (per-tier counts/oldest/newest), JSON-bearing sample count, archive and disk state. `db pre-image status|remove` (§7). `db archive status|export-now` (runs one cold-export pass and prints the manifest rows written). `config export [--out FILE]` (stdout by default; `--out` never overwrites an existing file), `config import FILE [--dry-run]` (prints the dry-run report; exit 1 on validation errors; a real import applies through `put_settings` and records the marker but **does not run maintenance** — the daemon prunes on its next tick, so a second process never runs the prune beside a live daemon; both verbs refuse a missing or `user_version 0` database like `db stats`).
