...
- **`auto` rule:** choose the finest tier such that (a) the tier is enabled, (b) `since_ms ≥ now − tier.keepDays` (the tier still holds the start of the range), and (c) `range_ms / resolution_ms ≤ limit`. If no tier satisfies (b), fall through to `Archive` when `archive.queryable`, else the coarsest enabled tier. Response gains `"source"` (already present) and `"resolutionMs"`.
