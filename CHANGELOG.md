...
- **A settings save now refuses a form it cannot read, instead of writing whatever the controls last held** (ADR 0034). `collectDaemonSettingsFromForm` reads 48 controls through an id-based cache taken at load, and a *detached* input still answers `.value` — verified in a browser: after removing a panel, `isConnected` is `false` and `.value` is still `"7"`. A missing one takes a fallback default. Either way the save serialised a value the user was not looking at and `PUT` it, with no exception, no failed request and no symptom until someone noticed their settings were wrong. 0.9.0 addressed this with a rule ("panels are hidden, never unmounted") and a test that greps the source; **a convention and a source-shape check are not a guarantee**, and unmounting an inactive panel is the obvious thing a future change would do. The save path now checks that every control it depends on reports `isConnected === true` and refuses by name if not. Proven end to end: with the Archive sub-panel removed from the document, the save produced **zero** `PUT`s and reported *"Nothing was saved: 4 settings could not be read — removed from the page after it loaded (Queryable archive, Cold CSV.gz archive, Cold after months, Archive directory)"*; the control case saved normally.
- **The check runs before any value is judged.** A range check on a detached input *passes* — it is checking the stale value — so continuing would produce a clean validation of data that never came from the form.
- **The manifest of guarded controls cannot fall behind the code**: a test extracts the `elements.*` set from the collect function and from the manifest and fails on any difference in either direction. Both drift classes were verified to fail it before shipping (adding a read without a manifest entry; deleting a manifest entry).
- **The retention ladder is now a list of tiers rather than a grid of boxes.** Four rows of `badge · resolution · on-off · span`, sharing explicit column tracks — not `auto`, which sizes per row and staggers them — so the columns read straight down and each tier's controls stay on its own row by construction. A switched-off tier recedes while its switch keeps full contrast. Row padding and gap are tuned to measurement: the earlier values overflowed a 720 px window by 9 px with the help paragraph open. Supersedes 0.9.0's two-column tier grid, which fixed the pairing but still read as eight unrelated boxes.
- **Fixed: the crate versions and both wrapper scripts were left at 0.8.2 when 0.9.0 shipped.** `/api/version` reads the `VERSION` file and correctly said 0.9.0, but `env!("CARGO_PKG_VERSION")` — which stamps the OTel `service.version` resource attribute and every exported settings document — still said 0.8.2. So exports misreported which build produced them. All four crates and both wrappers are now at the release version, and a new test asserts every restatement of the version agrees with the `VERSION` file, including that the newest CHANGELOG section matches. The bump had been a manual habit with nothing checking it, which is why it drifted the first time it was forgotten.

## 0.9.0 - 2026-09-01

The settings shell redesign (ADR 0033). Michel's four remaining instructions after using the deployed 0.8.1 dialog: redesign History, add a secondary row of tabs so nothing scrolls, do the same for Metrics, and add `(?)` help in sections and groups. (The fifth — the faulty toggle — shipped in 0.8.2.)

- **A settings panel with more content than a short viewport can hold now shows a secondary tab row.** General splits into Browser · Daemon · Thresholds · Display, History into Tiers · Archive · Disk, Advanced into Export · Document, and Metrics into the six `METRIC_REGISTRY` families built at runtime, so the sub-tabs cannot drift from the metrics they select. Thermals keeps no row because it already fits. **Every one of the 16 tab/sub-tab stops now measures overflow 0 at a 720 px window, including with a help paragraph expanded** — before this change General and Advanced scrolled at that height.
- **History is regrouped, and its tier rows are laid out in two fixed columns rather than `auto-fit`.** One flat fieldset held fourteen controls of three kinds, and because `auto-fit` wraps by rendered width, a switch for one tier landed between two *other* tiers' number fields. Each tier's enable switch now shares a row with its own days field by construction. Verified by geometry, not by eye: the four rendered rows are `L1 | L2`, `Enable L3 | L3 days`, `Enable L4 | L4 days`, `Keep L4 forever | Per-tick process history`.
- **The two tab rows are separate keyboard scopes.** Arrows move within the row that holds focus and never jump between rows; each row has its own roving `tabindex` and `aria-label`; the secondary selection is remembered per parent tab. Verified in a browser: arrows in History's row wrap without moving the primary tab, an arrow on the primary row moves primary and leaves History's sub-selection untouched, and returning to History lands on the sub-tab last used.
- **A sub-panel is hidden, never unmounted**, because `collectDaemonSettingsFromForm` reads through the id-based `elements` cache and a *detached* input still answers `.value` — unmounting would have kept saving, silently, with whatever a field held when it left the document. Proven end to end rather than asserted: four fields edited across three different sub-tabs, the dialog parked on a sub-tab holding none of them, and the captured `PUT /api/settings` body carried all four values.
- **Each settings group gains a `(?)` that expands a real paragraph in the DOM**, wired with `aria-expanded` and `aria-controls` rather than a `title` tooltip (invisible to touch, inconsistently announced). One paragraph per group, saying what the setting changes *and what it costs* — reasoning that has been written down in ADRs 0013–0021 since those decisions were made and had never reached the UI. Eight groups get one; OpenTelemetry, the raw document and CPU thermals deliberately do not, because they already carry permanently-visible warnings.
- **The OpenTelemetry group now lays out in columns.** ADR 0029 gave it one field per row because it shared the Advanced panel with the document editor and had half the width; with Advanced split into sub-tabs it owns the full width, and one field per row overflowed the short viewport by 43 px (measured). Supersedes that clause of ADR 0029.
- Metric family names now reach a DOM `id`, so they are folded to `[a-z0-9-]` and duplicates are suffixed — two families folding to one id would point two tabs at one panel and leave the other unreachable with no error.

## 0.8.2 - 2026-09-01

- **Every switch in the settings dialog was drawing two knobs** (ADR 0032). `styles.css` carried two switch implementations that could not override each other, because they drew the knob with *different pseudo-elements*: ADR 0028's `.settings-dialog input[type="checkbox"]` via `::after` at specificity (0,1,1), and a legacy v0.1.31 `.settings-group .toggle-row input[type="checkbox"]` via `::before` at (0,2,1), which won on track size and colour. Both matched every toggle, so all 13 rendered a 16 px knob travelling 18 px *and* a 12.79 px knob travelling 13.84 px — a 4.16 px offset when checked, which read as a smeared or half-thrown switch. Measured in a real browser before and after: **13 of 13 toggles painted two knobs; now 0 of 13.** The legacy block is deleted and ADR 0028's rule survives — it is the only one that also reaches the metric picker rows built at runtime — while adopting the legacy geometry (42 × 24 track, 16 px knob, 18 px travel), which is the size the dialog has used since v0.1.31 and the better pointer target.
- **The on-state switch colour now follows the theme accent instead of a pinned amber.** On `ember` this is a near-identical orange; on `solar`, `aurora`, `matrix` and the default it changes from orange to that theme's cool accent. This is deliberate: `.history-series-toggles` and `.inline-toggle` already use `accent-color: var(--cyan)`, so the dialog switches were the only accented control in a different hue. ADR 0032 carries an errata recording that its own first statement of this consequence was wrong for `solar`.
