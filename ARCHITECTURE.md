...
The daemon task set includes collection/history maintenance, disk checking, cold archive export, and the OTel exporter. Collection publishes each stored latest snapshot on a Tokio `watch` channel; the exporter task samples the most recent value at export time, applies configured resource attributes and environment-provided headers, and never delays collection when an export fails.
