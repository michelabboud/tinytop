...
async fn consecutive_legacy_edits_work_and_explicit_ladder_reset_wins() {
    ...
    let first_legacy_edit = DashboardSettings::from_document(first_document, Some(&previous))
    ...
    let second_legacy_edit = DashboardSettings::from_document(second_document, Some(&first))
...
async fn legacy_alias_edit_preserves_non_legacy_ladder_settings() {
    ...
    let legacy_edit = DashboardSettings::from_document(legacy_document, Some(&baseline))
