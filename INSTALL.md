...
Since 0.5.0, compiling the Rust daemon locally requires a C compiler on Linux, WSL, macOS, and Windows because `aws-lc-sys` builds native code. On Debian or Ubuntu, install `build-essential` (and optionally CMake) with `sudo apt install build-essential cmake`; on macOS with Homebrew, `brew install cmake` is harmless while the Xcode Command Line Tools provide the required compiler; on Windows, install Visual Studio Build Tools with the C++ workload, with CMake optional for normal builds and required only on the fallback paths below. `aws-lc-sys` tries its `cc` builder first when pregenerated bindings are available for the target. CMake is used only when explicitly selected (`AWS_LC_SYS_CMAKE_BUILDER=1`), for FIPS/no-assembly/sanitizer builds, for targets without pregenerated bindings, or after the `cc` builder fails. On Linux with `cc` absent, the real first `cc` 1.x failure line is `error occurred in cc-rs: failed to find tool "cc": No such file or directory (os error 2)`.
